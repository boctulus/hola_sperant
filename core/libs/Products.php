...
class Products
{
    ...
    */
    ...
    static function getLast($post_type = 'product', $post_status = 'publish'){
        global $wpdb;
    ...
    static function getFirst($post_type = 'product', $post_status = 'publish'){
        global $wpdb;
        ...
        $sql = "SELECT ID FROM `{$wpdb->prefix}posts` WHERE post_type = '$post_type' $include_post_status ORDER BY ID ASC LIMIT 1;";
    ...
    */
    ...
    static function getMetaValue($pid, $meta_key) {
        global $wpdb;
        ...
        $r = $wpdb->get_row($wpdb->prepare($sql, $pid, $meta_key), ARRAY_A);
    ...
    */
    static function setProductAttributesForSimpleProducts($pid, array $attributes, array $invisibles = []){
        $i = 0;
        ...
        foreach ($attributes as $name => $value) {
            $visibility = 1;
            if (!empty($invisibles)){
                $visibility = !in_array($name, $invisibles);
            }
            ...
            $product_attributes[$i] = array (
                ...
                'value' => $value, 
                'position' => 1,
                'is_visible' => $visibility, // <---------- debo poder especificar la visiblidad
                'is_variation' => 0,
                'is_taxonomy' => 0
            );
    ...
    */
    static function createOrUpdateCategoButNoOverwriteParentWithNullIfItHas($name, $slug = null, $description = null, $id_parent = null){
        if (!empty($id_parent)){
            return Products::createOrUpdateCatego($name, $slug, $description,  $id_parent);
        }

        // objeto de la categoria
        $_cat   = Products::getCategoryByName($name, ARRAY_A);
        ...
        if (empty($_cat) || (!empty($_cat) && empty($_parent))){
            $cid  = Products::createOrUpdateCatego($name, $slug, $description,  $id_parent);
        } else {
            $cid  = $_cat['term_id'];
        }
    ...
    */
    static function breadcrumb(int $cat_id){
     ...
     /*
        ...
        LEFT JOIN wp_terms as t ON tt.term_taxonomy_id = t.term_id 
        WHERE 1=1
    ...
    */
    static function getCategoriesFromProductId($pid){
        if (!static::productIDExists($pid)){
            return [];
        }

        // global $wpdb;

        // $sql = "SELECT t.term_id as cat_id FROM `{$wpdb->prefix}posts` as p
        ...
        // LEFT JOIN {$wpdb->prefix}terms as t ON tt.term_taxonomy_id = t.term_id
        // WHERE
        ...
        // $cat_ids = array_column($rows, "cat_id");
    ...
    */
    ...
    static function deleteAllProductCategories(){
        global $wpdb;

        $cats = static::getAllProductCategories();
